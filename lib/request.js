...
function request (connection, queueName, content, opts, cb) {
  ...
  if (cb) {
    // callback api
    connection.createChannel = pify(connection.createChannel.bind(connection))
  }
  var promise = connection.createChannel().then(function (channel) {
    if (cb) {
      // callback api
      channel.assertQueue = pify(channel.assertQueue.bind(channel))
      channel.consume = pify(channel.consume.bind(channel))
      channel.sendToQueue = pify(channel.sendToQueue.bind(channel))
      channel.deleteQueue = pify(channel.deleteQueue.bind(channel))
      channel.close = pify(channel.close.bind(channel))
    }
    var errData = {
      queue: queueName,
